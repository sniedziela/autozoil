...
sub process {
    ...
    open my $spellh, qq{echo '!' | cat - "$filename" | perl -pne 's/\\\\(eng|english|turkish|polish|ulurl|nolinkurl|reftext|mypicture|lstset|code){([^{}]|{[^{}]*})*}/ /g;s/\\\\index{[^{}]*}//g' | sed 's/^/^/' | hunspell -d $language -t -a |};
    binmode($spellh,':utf8');
